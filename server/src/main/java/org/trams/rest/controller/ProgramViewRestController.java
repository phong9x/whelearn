...
/**
 * Spring MVC controller for 'ProgramView' management.
 */
@RequestMapping("/programView")
@Controller
public class ProgramViewRestController extends AbstractRestController{
	...
	@RequestHeader(value="token") String token
		...
		try {
			List<ProgramView> list = programViewService.findAll();
			return SUCCESS(list);
		} catch (Exception e) {
	...
	@RequestBody String json
		...
		try {
			...
			Page<ProgramViewEntity> list = programViewService.findAll(page);
	...
	@RequestHeader(value="token") String token
		...
		//UserItem user =AuthorizationToken.convertToObject(token);
		try {
			ProgramView item= programViewService.findById(id);
			if(item!=null){
				return SUCCESS(item);
			}else{
				return SUCCESS();
	...
	try{
			...
			ProgramView create= programViewService.create(item);
	...
	try{
			...
			item.setUserId(user.getId());
			item.setUpdateDate(DataUtils.getNowDate());
			ProgramView edit=programViewService.update(item);
			return SUCCESS(edit);
		}catch(Exception e){
			System.out.println(e);
			return FAIL("HAVE ERROR: "+e);
